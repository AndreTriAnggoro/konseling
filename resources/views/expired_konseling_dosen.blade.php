        ...
        <div class="grid table-responsive">
            <table class="table table-stretched">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <!-- <td class="font-weight-medium">Ditolak pada {{ \Carbon\Carbon::parse($jadwal->created_at)->format('d F Y') }} Jam {{ \Carbon\Carbon::parse($jadwal->created_at)->format('H:i') }} dikarenakan {{ $jadwal->deskripsi }}</td></td> -->
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-end">
                    ...
                    @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $expired->previousPageUrl() }}" rel="prev">Previous</a>
                    </li>
                    @endif

                    @foreach ($expired->getUrlRange(1, $expired->lastPage()) as $page => $url)
                    <li class="page-item {{ $page == $expired->currentPage() ? 'active' : '' }}">
                    ...
                    <li class="page-item">
                        <a class="page-link" href="{{ $expired->appends(['search_column' => request('search_column'), 'search_query' => request('search_query')])->nextPageUrl() }}" rel="next">Next</a>
                    </li>
                    @else
                    <li class="page-item disabled">
                        <a class="page-link" href="{{ $expired->appends(['search_column' => request('search_column'), 'search_query' => request('search_query')])->nextPageUrl() }}" rel="next">Next</a>
